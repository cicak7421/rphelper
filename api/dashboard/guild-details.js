...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!supabaseUrl || !serviceKey) {
      return fail(res, 500, 'Supabase env belum lengkap.');
    }

    if (!botToken) {
      return fail(res, 500, 'Discord bot token env belum diisi.');
    }

    const body = req.body || {};

    if (!body.providerToken) {
      return fail(res, 400, 'Missing Discord provider token.');
    }

    if (!body.guildId) {
      return fail(res, 400, 'Missing guildId.');
    }

    const discordUser = await discordFetch('/users/@me', body.providerToken, true);

    if (!discordUser.ok || !discordUser.data?.id) {
      return fail(res, 401, 'Discord provider token invalid. Silakan logout lalu login ulang Discord.');
    }

    const guilds = await discordFetch('/users/@me/guilds', body.providerToken, true);

    if (!guilds.ok || !Array.isArray(guilds.data)) {
      return fail(res, 403, 'Gagal membaca daftar server user.');
    }

    const guild = guilds.data.find((item) => item.id === String(body.guildId));

    if (!guild) {
      return fail(res, 403, 'User tidak ada di server ini.');
    }

    if (!canManage(guild)) {
      return fail(res, 403, 'User bukan owner/admin/manage server.');
    }

    const botGuild = await discordFetch('/guilds/' + body.guildId, botToken, false);

    if (!botGuild.ok) {
      return fail(res, 403, 'Bot belum ada di server ini atau token bot tidak valid.');
    }
    ...
    await supabase.from('dashboard_guild_access').upsert(
      {
        user_id: 'discord_' + discordUser.data.id,
    ...
    return res.status(200).json({
      ...
      guild: {
        id: botGuild.data.id,
        name: botGuild.data.name,
        icon: botGuild.data.icon || null
      },
